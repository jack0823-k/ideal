...
<body align = "center">
		...
		<h2>
			<%= request.getParameter("rsvYy") %>年
			<%= request.getParameter("rsvMm") %>月
			<%= request.getParameter("rsvDd") %>日&ensp;
			<%= request.getParameter("rsvHh") %>時
			<%= request.getParameter("rsvMi") %>分より
		</h2>
		<!-- リクエストパラメータ等にコース名の情報が存在しないため、代わりにコースIDを表示します。→コース名を取得できるようにしました。 -->
		<h2>コース名：<%= request.getAttribute("courseName") %></h2>
		<h2>人数：<%= request.getParameter("person") %>名様</h2>
		<h2>ご予約番号は<%= request.getAttribute("rsvId") %>番です。</h2>
